            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">招聘展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="zhaopinName" id="zhaopinName" placeholder="公司名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>

                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label" style="width: 150px">招聘开始时间</label>
                          <div class="layui-input-inline">
                              <input type="text" name="zhaopinStartTimeStart" id="zhaopinStartTimeStart"  lay-skin="date"   placeholder="招聘开始时间开始" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <div class="layui-input-inline" style="margin-top: -5px">
                              <input type="text" name="zhaopinStartTimeEnd"   id="zhaopinStartTimeEnd"    lay-skin="date"   placeholder="招聘开始时间结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>

                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label" style="width: 150px">招聘结束时间</label>
                          <div class="layui-input-inline">
                              <input type="text" name="zhaopinEndTimeStart" id="zhaopinEndTimeStart"  lay-skin="date"   placeholder="招聘结束时间开始" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <div class="layui-input-inline" style="margin-top: -5px">
                              <input type="text" name="zhaopinEndTimeEnd"   id="zhaopinEndTimeEnd"    lay-skin="date"   placeholder="招聘结束时间结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../zhaopin/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.zhaopinPhoto?item.zhaopinPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../zhaopin/detail.jsp?id='+item.id)"><span>{{item.zhaopinName}}</span></div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 zhaopinContent 字段去除无用字符 如果字段长度大于60  就截取到60
            zhaopinContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#zhaopinName').val() != null && jquery('#zhaopinName').val() != "" && jquery('#zhaopinName').val() != "null" ) {
                param['zhaopinName'] = jquery('#zhaopinName').val();
            }
         
            if (jquery('#zhaopinStartTimeStart').val() != null && jquery('#zhaopinStartTimeStart').val() != "" && jquery('#zhaopinStartTimeStart').val() != "null" ) {
                param['zhaopinStartTimeStart'] = jquery('#zhaopinStartTimeStart').val();
            }
            if (jquery('#zhaopinStartTimeEnd').val() != null && jquery('#zhaopinStartTimeEnd').val() != "" && jquery('#zhaopinStartTimeEnd').val() != "null" ) {
                param['zhaopinStartTimeEnd'] = jquery('#zhaopinStartTimeEnd').val();
            }
     
            if (jquery('#zhaopinEndTimeStart').val() != null && jquery('#zhaopinEndTimeStart').val() != "" && jquery('#zhaopinEndTimeStart').val() != "null" ) {
                param['zhaopinEndTimeStart'] = jquery('#zhaopinEndTimeStart').val();
            }
            if (jquery('#zhaopinEndTimeEnd').val() != null && jquery('#zhaopinEndTimeEnd').val() != "" && jquery('#zhaopinEndTimeEnd').val() != "null" ) {
                param['zhaopinEndTimeEnd'] = jquery('#zhaopinEndTimeEnd').val();
            }
                    // 获取列表数据
            http.request('zhaopin/list', 'get', param, function(res) {
        ...
        function dateTimePick(){
            ...
            */
        // 招聘开始时间的开始时间和结束时间的效验及格式
        var zhaopinStartTimeStart = laydate.render({
            elem: '#zhaopinStartTimeStart'
            ,type: 'datetime'
            ,done: function (value, dates) {
                zhaopinStartTimeEnd.config.min={
                    year:dates.year,
        ...
        var zhaopinStartTimeEnd = laydate.render({
            elem: '#zhaopinStartTimeEnd'
            ,type: 'datetime'
            ,done: function (value, dates) {
                zhaopinStartTimeStart.config.max={
                    year:dates.year,
                    month:dates.month-1,//关键
                    date: dates.date,
                    hours:dates.hours,
        ...
        // 招聘结束时间的开始时间和结束时间的效验及格式
        var zhaopinEndTimeStart = laydate.render({
            elem: '#zhaopinEndTimeStart'
            ,type: 'datetime'
            ,done: function (value, dates) {
                zhaopinEndTimeEnd.config.min={
                    year:dates.year,
                    month:dates.month-1,//关键
                    date: dates.date,
                    hours:dates.hours,
        ...
        var zhaopinEndTimeEnd = laydate.render({
            elem: '#zhaopinEndTimeEnd'
            ,type: 'datetime'
            ,done: function (value, dates) {
                zhaopinEndTimeStart.config.max={
                    year:dates.year,
                    month:dates.month-1,//关键
                    date: dates.date,
                    hours:dates.hours,
