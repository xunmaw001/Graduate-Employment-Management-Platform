            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">宣讲会收藏展示</span>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../xuanjianghui/detail.jsp?id='+item.xuanjianghuiId)">
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../xuanjianghui/detail.jsp?id='+item.xuanjianghuiId)"><span>{{item.xuanjianghuiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteXuanjianghuiCollection(item.id)">删除 {{item.xuanjianghuiName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 xuanjianghuiCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            xuanjianghuiCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteXuanjianghuiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('xuanjianghuiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            // 当前表
                                // 获取列表数据
            http.request('xuanjianghuiCollection/list', 'get', param, function(res) {
