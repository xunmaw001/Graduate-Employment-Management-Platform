            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">就业指导展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label" style="width: 130px">就业指导名称</label>
                          <div class="layui-input-inline">
                              <input type="text" name="jiuyezhidaoName" id="jiuyezhidaoName" placeholder="就业指导名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>

                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label" style="width: 130px">就业指导类型</label>
                          <div class="layui-input-inline">
                              <select name="jiuyezhidaoTypes" id="jiuyezhidaoTypes" lay-filter="jiuyezhidaoTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in jiuyezhidaoTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../jiuyezhidao/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.jiuyezhidaoPhoto" />
                                        <h5>{{item.jiuyezhidaoName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.jiuyezhidaoContent|jiuyezhidaoContent}}...</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 jiuyezhidaoContent 字段去除无用字符 如果字段长度大于60  就截取到60
            jiuyezhidaoContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#jiuyezhidaoName').val() != null && jquery('#jiuyezhidaoName').val() != "" && jquery('#jiuyezhidaoName').val() != "null" ) {
                param['jiuyezhidaoName'] = jquery('#jiuyezhidaoName').val();
            }
     
            var jiuyezhidaoTypes = $("#jiuyezhidaoTypes").find("option:selected").val();
            if (jiuyezhidaoTypes != null && jiuyezhidaoTypes != "" && jiuyezhidaoTypes != "null") {
                param['jiuyezhidaoTypes'] = jiuyezhidaoTypes;
            }
                            // 获取列表数据
            http.request('jiuyezhidao/list', 'get', param, function(res) {
...
// 当前表的字典表
       ...
       function jiuyezhidaoTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jiuyezhidao_types", 'get', {}, function (res) {
               if(res.code == 0){
