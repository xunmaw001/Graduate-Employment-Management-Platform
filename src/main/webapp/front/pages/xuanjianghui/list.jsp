            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">宣讲会展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">宣讲会名称</label>
                          <div class="layui-input-inline">
                              <input type="text" name="xuanjianghuiName" id="xuanjianghuiName" placeholder="宣讲会名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>

                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label" style="width: 150px">宣讲会开始时间</label>
                          <div class="layui-input-inline">
                              <input type="text" name="xuanjianghuiStartTimeStart" id="xuanjianghuiStartTimeStart"  lay-skin="date"   placeholder="宣讲会开始时间开始" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <div class="layui-input-inline" style="margin-top: -5px">
                              <input type="text" name="xuanjianghuiStartTimeEnd"   id="xuanjianghuiStartTimeEnd"    lay-skin="date"   placeholder="宣讲会开始时间结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>

                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label" style="width: 150px">宣讲会结束时间</label>
                          <div class="layui-input-inline">
                              <input type="text" name="xuanjianghuiEndTimeStart" id="xuanjianghuiEndTimeStart"  lay-skin="date"   placeholder="宣讲会结束时间开始" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <div class="layui-input-inline" style="margin-top: -5px">
                              <input type="text" name="xuanjianghuiEndTimeEnd"   id="xuanjianghuiEndTimeEnd"    lay-skin="date"   placeholder="宣讲会结束时间结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../xuanjianghui/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.xuanjianghuiPhoto?item.xuanjianghuiPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../xuanjianghui/detail.jsp?id='+item.id)"><span>{{item.xuanjianghuiName}}</span></div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 xuanjianghuiContent 字段去除无用字符 如果字段长度大于60  就截取到60
            xuanjianghuiContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#xuanjianghuiName').val() != null && jquery('#xuanjianghuiName').val() != "" && jquery('#xuanjianghuiName').val() != "null" ) {
                param['xuanjianghuiName'] = jquery('#xuanjianghuiName').val();
            }
         
            if (jquery('#xuanjianghuiStartTimeStart').val() != null && jquery('#xuanjianghuiStartTimeStart').val() != "" && jquery('#xuanjianghuiStartTimeStart').val() != "null" ) {
                param['xuanjianghuiStartTimeStart'] = jquery('#xuanjianghuiStartTimeStart').val();
            }
            if (jquery('#xuanjianghuiStartTimeEnd').val() != null && jquery('#xuanjianghuiStartTimeEnd').val() != "" && jquery('#xuanjianghuiStartTimeEnd').val() != "null" ) {
                param['xuanjianghuiStartTimeEnd'] = jquery('#xuanjianghuiStartTimeEnd').val();
            }
     
            if (jquery('#xuanjianghuiEndTimeStart').val() != null && jquery('#xuanjianghuiEndTimeStart').val() != "" && jquery('#xuanjianghuiEndTimeStart').val() != "null" ) {
                param['xuanjianghuiEndTimeStart'] = jquery('#xuanjianghuiEndTimeStart').val();
            }
            if (jquery('#xuanjianghuiEndTimeEnd').val() != null && jquery('#xuanjianghuiEndTimeEnd').val() != "" && jquery('#xuanjianghuiEndTimeEnd').val() != "null" ) {
                param['xuanjianghuiEndTimeEnd'] = jquery('#xuanjianghuiEndTimeEnd').val();
            }
                    // 获取列表数据
            http.request('xuanjianghui/list', 'get', param, function(res) {
        ...
        function dateTimePick(){
            ...
            */
        // 宣讲会开始时间的开始时间和结束时间的效验及格式
        var xuanjianghuiStartTimeStart = laydate.render({
            elem: '#xuanjianghuiStartTimeStart'
            ,type: 'datetime'
            ,done: function (value, dates) {
                xuanjianghuiStartTimeEnd.config.min={
                    year:dates.year,
        ...
        var xuanjianghuiStartTimeEnd = laydate.render({
            elem: '#xuanjianghuiStartTimeEnd'
            ,type: 'datetime'
            ,done: function (value, dates) {
                xuanjianghuiStartTimeStart.config.max={
                    year:dates.year,
                    month:dates.month-1,//关键
                    date: dates.date,
                    hours:dates.hours,
        ...
        // 宣讲会结束时间的开始时间和结束时间的效验及格式
        var xuanjianghuiEndTimeStart = laydate.render({
            elem: '#xuanjianghuiEndTimeStart'
            ,type: 'datetime'
            ,done: function (value, dates) {
                xuanjianghuiEndTimeEnd.config.min={
                    year:dates.year,
                    month:dates.month-1,//关键
                    date: dates.date,
                    hours:dates.hours,
        ...
        var xuanjianghuiEndTimeEnd = laydate.render({
            elem: '#xuanjianghuiEndTimeEnd'
            ,type: 'datetime'
            ,done: function (value, dates) {
                xuanjianghuiEndTimeStart.config.max={
                    year:dates.year,
                    month:dates.month-1,//关键
                    date: dates.date,
                    hours:dates.hours,
