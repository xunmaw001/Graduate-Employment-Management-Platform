            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑咨询</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="laoshiId" name="laoshiId" type="hidden">
                                    <div class="form-group  col-md-6">
                                        <label>老师介绍详情</label>
                                        <input id="zixunContentupload" name="file" type="file">
                                        <script id="zixunContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var zixunContentUe = UE.getEditor('zixunContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="zixunContent" id="zixunContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#zixunContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('zixunContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#laoshiId") !=null){
            ...
            httpJson("zixun/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addzixun');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.zixunContent != null && ruleForm.zixunContent != 'null' && ruleForm.zixunContent != '' && $("#zixunContentupload").length>0) {

            var zixunContentUeditor = UE.getEditor('zixunContentEditor');
            zixunContentUeditor.ready(function () {
                var mes = '' + ruleForm.zixunContent;
                zixunContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#zixunContentupload").length>0) {
            var zixunContentEditor = UE.getEditor('zixunContentEditor');
            if (zixunContentEditor.hasContents()) {
                $('#zixunContent-input').attr('value', zixunContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                laoshiId: "required",
                zixunContent: "required",
            },
            messages: {
                laoshiId: "老师不能为空",
                zixunContent: "老师介绍详情不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addzixun = window.sessionStorage.getItem("addzixun");
        if (addzixun != null && addzixun != "" && addzixun != "null") {
            window.sessionStorage.removeItem('addzixun');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("zixun/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#zixunContent").val(ruleForm.zixunContent);
